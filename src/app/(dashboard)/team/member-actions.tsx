...
export default function MemberActions({ memberId, isActive, role, isOwner, isSelf }: Props) {
  ...
  const handleToggle = () => {
    // تعطيل منسق لا يُعيد إسناد عملائه — نحذّر المالك قبل التنفيذ
    if (isActive && role === "COORDINATOR") {
      const ok = confirm(
        "تعطيل هذا المنسق؟ عملاؤه النشطون سيبقون مسنَدين له — أعد إسنادهم من صفحة العملاء."
      );
      if (!ok) return;
    }
    setError("");
    startTransition(async () => {
      try {
        const result = await toggleTeamMember(memberId);
        if (!result.success) setError(result.error);
      } catch {
        setError("تعذّر تغيير حالة العضو — حاول مرة أخرى");
      }
    });
  };
  ...
  const handleRoleChange = (newRole: "ADMIN" | "COORDINATOR" | "PROVIDER") => {
    ...
    startTransition(async () => {
      try {
        ...
        setError("تعذّر تغيير الصلاحية — حاول مرة أخرى");
      }
    });
  };
  ...
  return (
    <div className="mt-3 pt-3 border-t border-surface-100">
      ...
      {error && (
        <p className="mt-2 text-xs text-danger-600 animate-fade-in">⚠️ {error}</p>
      )}
    </div>
  );
}
